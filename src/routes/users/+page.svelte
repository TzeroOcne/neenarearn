...
<div class="flex flex-col justify-center items-start max-w-2xl mx-auto mb-16">
  ...
  <div class="mb-8 prose leading-6 text-gray-600 dark:text-gray-400">
    <p>
      This page is setup in a way where you can programmatically route to sub routes. Click on each
      user card to go to there details view.
    </p>
  </div>
  {#if users.length > 0}
    {#each users as user}
      <a data-sveltekit-prefetch href={`/users/${user.id}`} class="w-full">
        <div
